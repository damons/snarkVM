...
pub enum InputID<A: Aleo> {
    ...
    Private(Field<A>),
    /// The `(digest, gamma, serial_number, tag)` tuple of the record input.
...
impl<A: Aleo> Inject for InputID<A> {
    ...
    fn new(_: Mode, input: Self::Primitive) -> Self {
        match input {
            ...
            // Inject digest and gamma as `Mode::Private`, and the expected serial number and tag as `Mode::Public`.
            console::InputID::Record(digest, gamma, serial_number, tag) => Self::Record(
                Field::new(Mode::Private, digest),
...
impl<A: Aleo> Eject for InputID<A> {
    ...
    fn eject_mode(&self) -> Mode {
        match self {
            ...
            Self::Record(digest, gamma, serial_number, tag) => {
                Mode::combine(digest.eject_mode(), [gamma.eject_mode(), serial_number.eject_mode(), tag.eject_mode()])
            }
    ...
    fn eject_value(&self) -> Self::Primitive {
        match self {
            ...
            Self::Record(digest, gamma, serial_number, tag) => console::InputID::Record(
                digest.eject_value(),
                gamma.eject_value(),
    ...
    fn to_fields(&self) -> Vec<Self::Field> {
        match self {
            ...
            InputID::Record(digest, gamma, serial_number, tag) => {
                vec![digest.clone(), gamma.to_x_coordinate(), serial_number.clone(), tag.clone()]
            }
