...
mod tests {
    ...
    #[test]
    fn test_identifier_to_field() -> Result<()> {
        let mut rng = TestRng::default();

        for _ in 0..ITERATIONS {
            // Sample a random identifier literal.
            let identifier = IdentifierLiteral::<CurrentNetwork>::rand(&mut rng);

            // Cast to field.
            let field: Field<CurrentNetwork> = identifier.cast()?;

            // Verify the field matches to_field.
            assert_eq!(identifier.to_field()?, field);
        }
        Ok(())
    }

    #[test]
    fn test_identifier_to_self() -> Result<()> {
        let mut rng = TestRng::default();

        for _ in 0..ITERATIONS {
            // Sample a random identifier literal.
            let expected = IdentifierLiteral::<CurrentNetwork>::rand(&mut rng);

            // Cast to self.
            let candidate: IdentifierLiteral<CurrentNetwork> = expected.cast()?;
