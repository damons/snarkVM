...
if [ "$TYPE" = "pr" ]; then
  ...
  # Truncation warning
  TRUNCATED=$(jq -s '[.[] | select(.comments.pageInfo.hasNextPage == true) | .path] | unique' "$WS/threads-pr-$NUM.jsonl")
  [ "$TRUNCATED" != "[]" ] && echo "WARNING: Threads in $(echo "$TRUNCATED" | jq -r 'join(", ")') have >100 comments (truncated)" >&2

  # Extract unresolved (grouped by file path, full conversations)
  jq -s '
    ...
    from_entries
  ' "$WS/threads-pr-$NUM.jsonl" > "$WS/unresolved-pr-$NUM.json"

  # Extract resolved (grouped by file path, full conversations)
  jq -s '
    ...
    from_entries
  ' "$WS/threads-pr-$NUM.jsonl" > "$WS/resolved-pr-$NUM.json"

  # Human-readable unresolved summary
  jq -r '
    to_entries[] |
    "== \(.key) ==",
    (.value[] |
      "  L\(.line // "?") [\(.comments[0].author // "?")]: \(.comments[0].body[0:200] | gsub("\n"; " "))",
      (if (.comments | length) > 1 then
        .comments[1:][] | "    -> [\(.author)]: \(.body[0:150] | gsub("\n"; " "))"
      else empty end)
    ),
    ""
  ' "$WS/unresolved-pr-$NUM.json" > "$WS/unresolved-pr-$NUM.txt"
  ...
  UNRESOLVED=$(jq '[.[]] | add // [] | length' "$WS/unresolved-pr-$NUM.json")
  RESOLVED=$(jq '[.[]] | add // [] | length' "$WS/resolved-pr-$NUM.json")
...
echo ""
echo "Files fetched:"
[ "$TYPE" = "pr" ] && echo "  - context, files, commits, comments, checks, threads, unresolved (.json + .txt), resolved, linked-issues, state"
