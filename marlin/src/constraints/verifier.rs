    ...
    PCG::CommitmentVar: ToConstraintFieldGadget<BaseField>,
{
    ...
    >(
        ...
        proof: &ProofVar<TargetField, BaseField, PC, PCG>,
        rng: &mut R,
    ) -> Result<Vec<snarkvm_polycommit::LinearCombinationVar<TargetField, BaseField>>, MarlinError<PC::Error>> {
        ...
        let eval = PCG::prepared_check_combinations(
            cs.ns(|| "prepared_check_combinations"),
            &prepared_verifying_key.prepared_verifier_key,
            &lc,
            &comm,
        ...
        println!("gadget evaluations_are_correct: {:?}", eval.get_value());
